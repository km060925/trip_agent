...
CITY_INFO = {
    ...
    "방콕": ("태국", "아시아", "Bangkok"), "푸켓": ("태국", "아시아", "Phuket"),
    "세부": ("필리핀", "아시아", "Cebu"), "마닐라": ("필리핀", "아시아", "Manila"),
    ...
    "타이베이": ("대만", "아시아", "Taipei"), "싱가포르": ("싱가포르", "아시아", "Singapore"),
    "괌": ("괌(미국령)", "아시아", "Guam"), "사이판": ("사이판(미국령)", "아시아", "Saipan"),
    # 유럽
    "파리": ("프랑스", "유럽", "Paris"), "런던": ("영국", "유럽", "London"),
    "로마": ("이탈리아", "유럽", "Rome"), "밀라노": ("이탈리아", "유럽", "Milan"),
    "프랑크푸르트": ("독일", "유럽", "Frankfurt"), "뮌헨": ("독일", "유럽", "Munich"),
    "취리히": ("스위스", "유럽", "Zurich"), "암스테르담": ("네덜란드", "유럽", "Amsterdam"),
    "바르셀로나": ("스페인", "유럽", "Barcelona"), "마드리드": ("스페인", "유럽", "Madrid"),
    "프라하": ("체코", "유럽", "Prague"), "빈": ("오스트리아", "유럽", "Vienna"), "비엔나": ("오스트리아", "유럽", "Vienna"),
    "이스탄불": ("튀르키예", "유럽", "Istanbul"), "아테네": ("그리스", "유럽", "Athens"), "리스본": ("포르투갈", "유럽", "Lisbon"),
    # 미주
    ...
    "라스베가스": ("미국", "미주", "Las Vegas"),
    "토론토": ("캐나다", "미주", "Toronto"), "밴쿠버": ("캐나다", "미주", "Vancouver"),
    "멕시코시티": ("멕시코", "미주", "Mexico City"), "상파울루": ("브라질", "미주", "Sao Paulo"),
    # 오세아니아
    "시드니": ("호주", "오세아니아", "Sydney"), "멜버른": ("호주", "오세아니아", "Melbourne"),
    "브리즈번": ("호주", "오세아니아", "Brisbane"), "오클랜드": ("뉴질랜드", "오세아니아", "Auckland"),
}
...
def _clean_agoda_hits(hits: list, url_marker: str, required_terms: Optional[list] = None) -> list:
    """아고다 검색 결과 중 실제 상세 페이지(호텔/항공편 라우트 페이지)만 남기고,
    ...
    for hit in hits:
        ...
        for marker in _AGODA_NOISE_MARKERS:
...
def search_hotels(city: str, check_in: str, check_out: str, guests: int) -> str:
    ...
    try:
        ...
        hits = search_result.get("results", []) if isinstance(search_result, dict) else []
        cleaned = _clean_agoda_hits(hits, "/hotel/", required_terms=[city, city_en])
